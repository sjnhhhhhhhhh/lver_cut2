...
void STL2OFF(const std::string& stl_filename, const std::string& off_filename) {
    ...
    off_file.close();
    cout << "Conversion to OFF completed successfully!" << std::endl;
}

int main() {
    string stl_filename = "C:/code/liver_cut/data/Hepatic_portal_vein.stl";
    string off_filename = "C:/code/liver_cut/trans_data/Hepatic_portal_vein.off";
    STL2OFF(stl_filename, off_filename);
